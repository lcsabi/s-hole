...
- **DNS-over-HTTPS (DoH) or DNS-over-TLS (DoT) termination (serving encrypted DNS to LAN clients).** The LAN-facing listener is plain UDP and TCP on port 53. Encrypted *upstream* forwarding is supported (DoH upstream, CL 85; see the Request-processing pipeline). Serving DoT or DoH to clients is separate follow-up work (`docs/ROADMAP.md` #36).
