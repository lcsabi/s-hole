...
- **`config.yaml` is a normally-tracked file; edit and commit it like any other.** The committed sample must keep conservative defaults (`query_db: "queries.db"`, `api_listen: "127.0.0.1:8080"`). Never commit `query_db: ""` or `api_listen: "0.0.0.0:8080"`: they ship the unauthenticated admin UI LAN-exposed, and the release archives bundle this file. For local testing overrides, use the `S_HOLE_*` environment variables instead of editing the committed file (`internal/config` reads them over the YAML): `S_HOLE_API_LISTEN=0.0.0.0:8080` for the LAN dashboard and `S_HOLE_QUERY_DB=` for SQLite off. The old `--skip-worktree` "dance" was removed on 2026-09-08; there is no longer a hidden local divergence, so an intended change to the sample now shows in `git status` and commits normally.
...
- **Single-flight reload**: one `TryLock` closure wraps the reload work (`reloadWork`: the DoT certificate when DoT is on, then `blocklist.Update`); the periodic ticker, `POST /api/reload`, and SIGHUP (also sent by `systemctl reload` through the unit's `ExecReload`) all go through it. The mutex must stay in main; a mutex inside `api` was a P0 once (b/022) because the ticker bypassed it.
...
- **Package-level test seams** (`blocklist.maxBodyBytes`, the `swapLogger` logger swap in `blocklist`) are mutated only by tests and are safe only because each package runs its tests sequentially (no `t.Parallel`). Production never writes them. If you add `t.Parallel` to a package that mutates a package-level var, pass the value in instead of mutating the global, or the write races the production read under `-race`.
...
- **End every non-trivial change with a repo-wide documentation and comment sanity check, run by a fresh generic sub-agent.** After the code and the per-file doc syncs are in, delegate one complete pass to a cold `general-purpose` sub-agent (not a `fork`: a fork inherits this session's context and carries the author's assumptions, which defeats the point). It reads the docs and comments as a new reader and checks: consistency (every place that quotes a changed fact agrees, including the frequent-duplicates list below), up-to-date and desired-state (docs, comments, CHANGELOG, CL.md, ROADMAP reflect the current intent), STE compliance for operator/maintainer-facing material, and comment quality (new comments match the local density and idiom, none describe old behavior). The sub-agent reports findings; apply the fixes.
- **Open a PR; do not merge.** Do the work on a feature branch and open a pull request for review. Do not merge it. The maintainer reviews and tests the build (often on a VM) before merging.
...
- **Writing style (STE-like)**: docs, code comments, and commit messages use short, active, plain sentences (Simplified Technical English in spirit). **No em-dashes (`—`)** in living files; use a period, colon, semicolon, or parentheses instead. Put the condition before the command in procedures. `docs/cls/CL-*.md` and `docs/BUGS.md` are immutable and exempt; new CL titles use `# CL N: title` (a colon, not an em-dash). The anti-slop tells (em-dashes, the triadic "A, B, and C" cadence, "not just X but Y" balancing, empty intensifiers) apply everywhere, design rationale and "why" comments included. The nuance exemption for rationale covers only mechanical brevity (no hard sentence-length cap, no approved-word list); keep the reasoning that makes those parts worth reading. This is a taste guideline to keep the AI-slop smell down, not a CI-enforced gate: the aim is that em-dashes stay rare, not that a build fails on one. Normal dashes are fine (hyphens, and en-dash ranges like `2000–5000`). `grep -n '—' <file>` is a convenience for the obvious cases, not proof: on the maintainer's WSL box `grep` is ugrep and has silently missed em-dashes, so read the changed prose rather than trusting a clean grep. Treat `/simple-english` as a manual tool for operator text (Quick Start, deploy steps, error strings, installer echoes, dashboard labels and empty states), not the enforced whole-repo standard; do not run it over rationale or "why" comments. STE compliance for operator- and maintainer-facing material is about understandability, not only removing the anti-slop tells: it also covers wording and readability (short active sentences, condition before command, one meaning per word, the can/will/must modal ladder). The `/simple-english` skill holds those rules and the self-check; run new operator-facing strings through it, not merely the em-dash scan.
