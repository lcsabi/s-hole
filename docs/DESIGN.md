...
An optional DNS-over-TLS listener (RFC 7858; CL 86) serves LAN clients that want an encrypted channel. It is aimed at Android's default Automatic Private DNS mode, which uses DoT on the network's DNS server when it answers and does not check the certificate, so any certificate works and the phone needs no setup. Strict mode (a provider hostname) also works, but the phone then checks the certificate, so it needs a publicly trusted certificate for a name the phone can resolve. It is off unless `dot_listen` is set. It is a third `dns.Server` (`Net: "tcp-tls"`) that dispatches to the same handler as the plain listeners, so blocking, the cache, stats, the query log, and the `query_privacy` mask apply unchanged; `clientAddr` still sees the client's `*net.TCPAddr` under the `tls.Conn`. `main` binds the port itself (`dnsserver.ListenDoT`) right after config validation, before anything else opens, and hands the pre-bound listener to miekg/dns, which serves it through `ActivateAndServe`. A bind or certificate failure is fatal. The admin UI fails open because it is not the DNS service, but an enabled DoT listener that did not come up would silently cut off the clients that depend on it. Under the TLS layer, a small limiter caps open connections at 256 (a constant, not a knob). miekg/dns's 2 s per-connection read timeout also bounds the TLS handshake, which runs inside the first read, and its 8 s idle timeout and 128-query cap bound each connection. TLS 1.2 is the floor. The operator supplies the certificate (`tls_cert`, `tls_key`). Only the operator knows the hostname clients connect with (the SAN) and can make clients trust the issuer, and a public CA cannot issue for a private name, so s-hole generates nothing. The certificate is served through `tls.Config.GetCertificate` from an atomic pointer (`dnsserver.CertReloader`). The single-flight reload path swaps it, so a renewal reaches new connections without a restart; open connections keep the old certificate until they reconnect, and a reload whose files do not load keeps the current certificate. s-hole logs a WARN at startup and on each reload when the certificate has expired or expires within 14 days. The DoT path has not yet been tested with a real Android device; `docs/ROADMAP.md` #36 tracks that acceptance test.

The `Handler` struct is the core routing point. For each query:

1. Extract the question's domain name and client IP. The client IP is then masked once, per the `query_privacy` mode, at this single write-time choke point (`raw` keeps it, `drop` stores nothing, `subnet` masks to IPv4 /24 or IPv6 /64). Because the mask runs before step 3, the stats counter (Top Clients) and every query-log sink record the same masked value, and no unmasked address is retained anywhere. Masking is forward-only.
...
An upstream is either a plain resolver (`host:port`, queried over UDP with a TCP retry when the reply is truncated) or a DNS-over-HTTPS endpoint (`https://<IP>/dns-query`, RFC 8484; CL 85). A DoH upstream carries the same wire-format query in an HTTPS POST over port 443, so the ISP interception and on-path tampering that plain port-53 traffic is exposed to no longer apply to the upstream hop. Plain and DoH upstreams share one ordered list and the same failover, cooldown, and per-upstream failure metric, because those all key off the upstream string; an operator lists a DoH endpoint first and a plain resolver after it to keep a fallback. A DoH endpoint must use an IP host, not a hostname: s-hole is often the LAN's own resolver, so resolving a DoH hostname could loop back into s-hole, and an IP host removes that bootstrap lookup (the major providers' certificates carry IP SANs, so TLS still verifies against the IP). DoH reuses the 3-second per-attempt budget; one shared HTTP client keeps the TLS connection alive, so only the first query after startup or a long idle gap pays the handshake. This is upstream (client) DoH only; serving DoH to LAN clients is out of scope (see Non-Goals and `docs/ROADMAP.md` #36).
...
The store's memory grows linearly with the number of unique domains, while lookup time does not grow with it at all. The blocked set is a `map[string]struct{}`, so each entry costs the Go map overhead plus the domain bytes, and the empty-struct value adds nothing. Measured, that is roughly 70 to 100 bytes per entry. It works out to about 5 MB for an 80k list, 20 MB for a 250k list, and 200 MB for a 2.26M-entry list such as HaGeZi's full threat-intelligence feed. Lookup cost stays flat because the suffix walk does O(labels) probes regardless of how many domains the map holds; a larger map only adds a small constant CPU-cache penalty per probe, never more probes. Two effects sit above this resting footprint. First, `Store.Replace` builds the whole new map before it swaps the pointer, so a refresh holds the old and new sets at once and briefly doubles the set's memory (plus the transient download and parse buffers). Second, because the in-memory set deduplicates, overlapping lists cost only their union, so stacking lists is cheaper than the sum of their sizes suggests. At the tiers the sample config recommends (up to about 250k domains) both figures are small on home hardware. The full threat-intelligence feed is the one to watch on a small Raspberry Pi, and for memory, not for lookup speed.

For operator sizing, the figure to budget is the whole process, not the set alone. Measured resident memory (RSS) on a 64-bit Linux build was about 28 MB for an 80k list, 45 MB for a 320k list, and 79 MB for a 440k list: the set from the paragraph above, plus the Go runtime baseline (a few MB) and the response cache. These are idle figures just after startup. A blocklist refresh adds the reload transient on top for a few seconds (`Store.Replace` holds the old and new sets at once; see `docs/ROADMAP.md` #32). All of these fit comfortably on a Raspberry Pi. Method: `VmRSS` from `/proc/<pid>/status` with the sample config on Go 1.26.x. Treat the numbers as order-of-magnitude guidance, not a contract; they move with the Go version and the exact lists.
...
2026-06-23T10:04:06Z ALLOW 192.168.1.42 cdn.example.com. CACHED
2026-06-23T10:04:07Z ALLOW 192.168.1.42 dead.example.com. FAILED
```
An ALLOW line carries an optional trailing marker: ` CACHED` for a reply served from the response cache, or ` FAILED` for a failed query (s-hole synthesized a SERVFAIL, or relayed a SERVFAIL/REFUSED from an upstream). The two markers are mutually exclusive, and a BLOCK line carries neither (a blocked query never reaches the cache and its rcode is not a failure). Suitable for `grep`, `tail -f`, and log rotation via external tools (e.g. `logrotate`).
...
The `querylog.Logger` interface (`Log(rec Record)`) is implemented by `FileLogger`, `DBLogger`, and `Multi`, with compile-time assertions in the package so a future signature drift is caught at build time rather than at the call site. `Record` is a struct rather than positional arguments, so a new per-query field does not churn every implementation. It carries the client, domain, and blocked flag, plus `CacheHit` (CL 76) and the outcome pair `Rcode`/`Synthesized` (CL 77): `Synthesized` is true when s-hole built the reply itself (a block, a local PTR answer, or a synthesized SERVFAIL) and false when it relayed an upstream or cached message, so the `Failed`/`Unresolved`/`UpstreamError` helpers derive the outcome without a separate stored enum.
...
`Snapshot(topN int)` returns a `Summary` struct with json tags, making it directly serialisable by the REST API without coupling the stats package to any HTTP library. Fields include uptime, totals, block percentage, local-PTR count (`local_ptr_count`, queries answered locally per RFC 6303), cache hit count and percentage, the two failure counters (`forward_failures`, queries s-hole could not resolve; `upstream_errors`, failure rcodes relayed from a live upstream), blocklist size (`blocklist_size`, set by the API handler after calling `Snapshot`, since the stats package does not depend on the blocklist package), and top-N entry lists. The two failure counters obey the same load-order invariant as the other per-query counters (read before `total` in `Snapshot`, since the handler bumps them after `RecordQuery`). The per-source blocklist health (`sources`, an array of `{url, count, last_refresh, stale}`) is added the same way: the api handler wraps `Summary` in a `statsResponse` and fills `sources` from `blocklist.Store.Sources()`, so the stats package still takes no blocklist dependency. `count` is the pre-dedup domain count each source contributed, so the sum exceeds `blocklist_size`; `stale` is true while a source is served from its on-disk cache after a failed fetch, or has never loaded. The active `query_privacy` mode (`raw`, `drop`, or `subnet`) rides on the same `statsResponse` wrapper so the dashboard can describe the client column; it is display metadata only, since the client IP is already masked in the DNS handler. The DNS-over-TLS state rides on the same wrapper as a `dot` object (`{"enabled": false}` while DoT is off). `main` fills it from `dnsserver.CertReloader.Status` through `api.Server.SetDoTStatus`, so the api package does not import dnsserver; the state (`ok`, `expiring`, `expired`, `reload_failed`) is computed in dnsserver, where the 14-day window is defined. Each top-client entry also carries an optional `label` from the `client_names` map (CL 73), resolved in the handler against the masked client value; `/api/queries` rows carry the same `label`, plus a computed `outcome` label (`allowed`/`blocked`/`unresolved`/`upstream_error`) derived once server-side by `QueryRow.Outcome` from the stored rcode and synthesized flag, so the dashboard and any export read a name instead of decoding the rcode. Both are added in the api layer, so the `stats` and `querylog` packages are untouched.

`Snapshot` loads `blocked`, `localPTR`, `cacheHit`, `forwardFailures`, and `upstreamErrors` all *before* `total`. This load order matters: `RecordQuery` increments `total` atomically *before* taking the mutex and incrementing `blocked`, so reading `total` first allows concurrent queries to inflate `blocked` past the snapshotted `total` and yield a block percentage greater than 100. Reading `blocked` first guarantees the invariant `blocked ≤ total`. `localPTR`, `cacheHit`, and the two failure counters are also incremented after `total` (by `RecordLocalPTR`, the cache-hit path, and the forward path, all after `RecordQuery`), so each must be read before `total` for the same reason. The cache-hit-rate denominator is `total − blocked − localPTR` because neither blocked nor local-PTR queries ever reach the cache or the upstream.
...
2. **Web UI.** A single-page dashboard embedded in the binary via `//go:embed`. It polls `/api/stats` and `/api/queries` every 3 seconds and renders stat cards, a "Queries over time" graph, top domain/client tables, a per-source blocklist health panel (URL, domain count, last refresh, and an OK/STALE badge), an actions panel (reload, whitelist add, and a "why is this blocked?" domain check that shows the decision and the full suffix walk), and a recent query log. The Top Blocked Domains panel has a "Since start / All time" toggle: "Since start" reads the in-memory `top_domains` tally from `/api/stats` (resets on restart, caps at `topNMaxEntries`), while "All time" polls `/api/top-blocked` for the persistent SQLite tally. The "Queries over time" graph draws a canvas chart from `/api/history` with a 24h / 7d toggle: total (violet), blocked (red), cached (teal), and the two failure lines, unresolved (amber) and upstream error (orange), per bucket. Because it reads the query log, it adapts to `log_queries`: the endpoint reports the effective mode, so the graph shows all lines under `all`, a single labeled blocked line under `blocked` (the log holds only blocked rows, so total, cached, and the failure lines are hidden), and an empty state under `none` or when `query_db` is unset. The failure lines are amber and orange (not red, which means blocked) so a spike reads as trouble; each is a subset of the forwarded remainder, so both scale under the total envelope. The cached line is the subset of total served from the response cache, so the cache hit rate reads off the chart as the ratio of the cached line to the total line. Cache-hit recording is forward-only: rows written before the feature landed read as not-cached, so the cached line under-reports for earlier buckets. This is also why the graph can differ from the stat cards, which count every query from the in-memory counter regardless of `log_queries`. The Top Clients panel adapts to `query_privacy` the same way: it reads the mode from `/api/stats` and, under `drop`, shows a "clients not recorded" state instead of blank rows, and under `subnet` notes that the entries are subnet-masked. When a `client_names` label resolves for a client, both the Top Clients panel and the recent-query log show the label with the masked IP as a small subtext line beneath it; an unlabeled client shows the IP alone. The recent-query log has filter controls in its header: a domain substring box, an All/Blocked/Allowed/Unresolved/Upstream-error toggle, and a client picker seeded from the Top Clients list. The failure segments map to `?outcome=`, the block segments to `?blocked=`. Each matching row shows a status badge (BLOCK, ALLOW, UNRESOLVED, or UPSTREAM ERR). The filter runs server-side in SQL, so it matches only the stored (masked) columns. The client picker follows `query_privacy`: it is hidden under `drop`, where no client is stored. The header also has Export CSV and Export JSON links that download the current filter through `/api/queries/export` (the same filter params, without the row cap); they grey out when the query log reports it is off or `none`, so an empty download is not offered. The active filter persists in `localStorage`. A filter row and a Clear button show while a filter is set, and the empty state names the filter, so a restored filter is never mistaken for a broken log. The dashboard header shows the active `query_privacy` mode at all times. When DNS over TLS is on, the header also shows a certificate badge from the `/api/stats` `dot` object: OK (green) with the days left, EXPIRES SOON (amber) inside 14 days, EXPIRED or RELOAD FAILED (red); its tooltip lists the certificate names, the expiry, and the last reload error. The server computes the state and the days left, so the browser clock does not matter. The three panels that fetch their own data (Queries over time, Recent Queries, Top Blocked) are collapsible; a collapsed panel hides its body and skips its poll, and the choice persists in the browser's `localStorage`.
...
Reloads are single-flighted via a `sync.Mutex` held in `cmd/s-hole/main.go` and shared between the periodic refresh timer, the API, and SIGHUP. A reload re-reads the DoT certificate (when DoT is on), then refreshes the blocklists; a certificate that fails to load does not skip the blocklist refresh. The reload closure tries to acquire the lock and returns `true` synchronously if it took it (work proceeds asynchronously in a goroutine) or `false` if a reload is already running. `POST /api/reload` surfaces the boolean as `"reload triggered"` vs `"reload already in progress"`. Centralising the lock in the closure rather than in `api.Server` ensures the periodic timer cannot bypass the gate.

State-changing admin requests leave an audit line in the application log. A whitelist add or remove logs the domain and the requester address at `Info`. Each reload logs its trigger source: `via API` with the client address, `via timer`, or the existing SIGHUP `reload signal received` line. The admin API is unauthenticated on the LAN, so a whitelist change un-blocks a domain for the whole network. The log is the record of who changed what and when. slog writes it to stdout (journald captures it under systemd, and the Windows Event Log captures it under the SCM), and s-hole keeps no separate audit store.

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/api/stats` | GET | Live stats snapshot (uptime, totals, cache rate, blocklist size, per-source blocklist health, top domains/clients, active `query_privacy` mode, and a `dot` object with the DNS-over-TLS certificate state) |
...
| `/api/queries` | GET | Recent queries from SQLite (`?limit=N`, default 50, capped at 1000; filters `?domain=` substring, `?client=` exact, `?blocked=true`/`false`, `?outcome=unresolved`/`upstream-error`) |
| `/api/queries/export` | GET | Stream the filtered query log for download (`?format=csv` default, or `json`; same filters as `/api/queries`; uncapped unless `?limit=N`). Empty valid file when `query_db` is unset |
...
| `/api/history` | GET | Per-bucket total, blocked, cached (cache-hit), unresolved, and upstream-error query counts over a window from SQLite (`?window=24h&bucket=1h`, defaults 24h/1h, bucket count capped at 1000); empty series when `query_db` is unset |
...
| `/api/reload` | POST | Trigger an immediate reload: re-read the DoT certificate (when DoT is on), then refresh the blocklists (de-duplicated via single-flight mutex) |
...
| `/metrics` | GET | Prometheus text exposition (format `0.0.4`) of the `shole_*` series. See the Metrics reference table below for the full list. |
...
| `shole_dot_certificate_expiry_timestamp_seconds` | gauge | Unix time when the DNS-over-TLS certificate being served expires, so an alert can compare it with `time()`. Present only when `dot_listen` is set. |
| `shole_dot_certificate_reload_failures_total` | counter | Reloads whose DoT certificate files did not load; s-hole kept serving the previous certificate. Present only when `dot_listen` is set. |
...
| `shole_goroutines` | gauge | Live goroutine count, read from `runtime/metrics` at scrape time. s-hole runs one goroutine per in-flight query, so a value that climbs and never settles is the goroutine-leak signal. |
| `shole_memory_alloc_bytes` | gauge | Bytes of allocated heap objects (live and not-yet-freed), from `runtime/metrics` `/memory/classes/heap/objects:bytes` (equals `runtime.MemStats.HeapAlloc`). |
| `shole_memory_heap_inuse_bytes` | gauge | Bytes in in-use heap spans (allocated objects plus span fragmentation): the `/memory/classes/heap/objects:bytes` and `/memory/classes/heap/unused:bytes` classes summed (equals `runtime.MemStats.HeapInuse`). |

The three runtime gauges are read from the sampled `runtime/metrics` API, once per scrape, so a scrape never triggers a stop-the-world pause. They are read at scrape time only, never on the query path.
...
- **`/metrics`**: Prometheus text exposition (format `0.0.4`) covering query counts, cache activity and back-pressure, blocklist size and per-source health, whitelist size, upstream failures, query-log back-pressure, the DoT certificate (when DoT is on), and Go runtime gauges (goroutines and heap, read from the sampled `runtime/metrics` API for process leak and heap-growth watching). The Metrics reference table above lists every metric with its type and meaning. We hand-roll the exposition rather than pulling in `prometheus/client_golang` to keep the dependency graph small.
...
On startup, `cmd/s-hole/main.go` calls `printNetworkHint`, which enumerates the local interfaces via `net.Interfaces()`, keeps IPv4 addresses on interfaces that are up and are not loopback, link-local, or a container, VM, or VPN interface (Docker, libvirt, VirtualBox and VMware host-only networks, LXC/LXD/Incus, Podman, Kubernetes network plugins, WireGuard, Tailscale, ZeroTier, tun/tap; matched by name, because their addresses are ordinary private ones, b/059), and prints a bordered box listing the DNS server address for each remaining address. The Admin UI line honors where the API is actually bound (T4): with the localhost-only default it prints a single `http://127.0.0.1:<port> (this machine only)` line rather than advertising LAN URLs that would refuse connections. With DoT on, one more line names the DoT port and not an address, because a DoT client connects by the hostname in the certificate, not by IP. This removes the need for the operator to manually discover the machine's IP when configuring the router's DHCP DNS field. `deploy/install-linux.sh` prints the same banner at the end of installation from `ip -4 -o addr show up scope global` with the same interface-name filter (falling back to `hostname -I`), and the same `api_listen`-aware Admin UI logic.
...
A `Validate()` method runs after `Load()` and rejects unrecognised values for the enumerated fields (`block_mode`, `log_queries`, `query_privacy`). A typo such as `block_mode: "NXDOMAIN"` is now a fatal startup error instead of a silent fallback to the default, so operator misconfiguration is surfaced immediately at the source. `Validate()` also checks the upstream list. `Load()` first drops any upstream that is not `host:port` (with a WARN, the same drop-and-warn approach the whitelist and `client_names` loaders use for their own checks), so an empty list at `Validate()` means every configured upstream was malformed; that is fatal, because a config that cannot forward at all should fail `-check-config` and startup the way a bad `block_mode` does. A single valid upstream is accepted but logs an INFO note, since it leaves no forwarding fallback; a deliberate single local resolver is a valid setup, so the note never blocks startup. The check is `net.SplitHostPort` shape only: it resolves no name and dials nothing, so it adds no network call and does not probe an operator-supplied address (which, wired to the unauthenticated admin API, would be an SSRF and LAN-scanning primitive). A well-formed but dead or typo'd address stays a runtime concern the upstream cooldown tracker already handles. When `dot_listen` is set, `Validate()` also requires `tls_cert` and `tls_key` and loads them with `tls.LoadX509KeyPair`, so `-check-config` catches an unreadable or mismatched pair before the service starts. This reads two local files and makes no network call.
...
s-hole supports these deployment targets:
...
Linux is the primary deployment target: the Raspberry Pi optimisations, the hardened systemd unit, and the Docker image are all built around it; Windows is supported (`-service install` and SCM integration) but is not the design's centre of gravity. Accordingly, `SIGHUP` is wired up as the conventional reload gesture on every non-Windows build: `kill -HUP $(pidof s-hole)` (or `systemctl reload s-hole`) triggers the same single-flight reload as `POST /api/reload`. That reload re-reads the DoT certificate when DoT is on and re-downloads the blocklists from the URLs read at startup; it does not re-read `config.yaml`, so a config change takes effect on the next restart. Operators get the muscle-memory behaviour even when the admin API is disabled or firewalled.

The implementation lives in two tiny build-tagged files (`cmd/s-hole/signals_unix.go` and `cmd/s-hole/signals_windows.go`) so `cmd/s-hole/main.go` itself contains no platform-specific code. On Windows, `reloadSignals()` returns nil and the only signals notified are SIGINT/SIGTERM. The SCM is the canonical lifecycle control there, and POST /api/reload remains available for an on-demand reload.
...
- **`client_names` labels** (CL 73) add device-identity PII to the unauthenticated admin API: an IP that a reader had to correlate becomes a named device. This opens no new hole (there is no new endpoint and no wire-parsed input; the map is parsed once at config load), and it cannot exceed the active `query_privacy` granularity, because the label is resolved from the stored (already masked) client value: under `subnet` only a CIDR or network-address key resolves, and under `drop` nothing does. There is no path from a label back to a device the mask hid. Labels are HTML-escaped in the UI, so an operator-supplied label cannot inject markup. The mitigation is the same as for the rest of the admin API: keep `api_listen` on localhost or a trusted LAN.
...
The `query_privacy` setting (CL 72) controls how much of the client IP is stored: `raw`, `subnet` (host bits masked), or `drop` (no client). The `client_names` map (CL 73) is a read-only display label resolved against that stored value, so it tracks the privacy level rather than defeating it: a host label resolves only under `raw`, a subnet/CIDR label under `subnet`, and nothing under `drop`.

---

## Testing Strategy

- **Unit tests:** Every implementation package under `internal/` ships a `*_test.go` file. Coverage targets (checked in review, not a strict CI gate): `stats`, `config`, and `version` 100 %; `cache` ≥ 94 %; `api`, `blocklist`, `dnsserver`, and `querylog` ≥ 85 %. The `cmd/s-hole` bootstrap (the `main()` wiring and signal-dispatch goroutine) and the platform-specific `internal/service` glue run below these targets because they need a running binary or Windows; both are exercised by manual smoke tests. Run `go test -cover ./...` for the current numbers.
- **What the unit tests exercise:** `blocklist.Store` lookup, suffix (subdomain) blocking and whitelist suffix precedence, atomic `Replace`, the `Explain` diagnostic walk (decision plus matched block/whitelist entries), `parseHostsFormat` against both formats, `Update` preserving the store on full-failure refresh, raising the empty-block-set alarm when a refresh loads nothing, and recording per-source status (healthy plus hard-failure), `ValidDomain` rejecting garbage, atomic cache file write; `cache.Cache` TTL decrement, drop-on-full (with the drop counter and expired-slot reclaim on a full insert), Qclass-aware keying, `cleanupExpired` sweep, `Close` shutdown; `config.Load` with empty/partial/invalid YAML dropping invalid whitelist entries and invalid `client_names` keys, `Validate` rejecting bogus enums, the DoT checks (address shape, cert and key required, an unloadable or mismatched pair rejected, TLS paths ignored while DoT is off), accepting a DoH IP-literal upstream and dropping a hostname or non-https DoH URL, every duration-parser error path, every `S_HOLE_*` env override; `stats.Counter` concurrent invariants (block rate never exceeds 100 % under parallel writers, `local_ptr_count` never exceeds `total_queries` under parallel PTR writers, cache hit rate never exceeds 100 % under parallel cache-hit writers, `forward_failures` and `upstream_errors` never exceed `total_queries` under parallel failure writers), top-N map cap, `Print` output; `querylog.FileLogger` filtering modes + fallback paths and the ` FAILED` marker, the `Record` failure-derivation helpers, `DBLogger` round-trip, final-flush-on-Close, retention prune, the `Search` filter (domain substring, exact client, blocked flag, `?outcome=` failure filter, and `Recent` equivalence), the streaming `Export` (filter parity with `Search`, uncapped and limited, stop-on-yield-error, context cancellation), the outcome-column round-trip and the idempotent `rcode`/`synthesized` migration; `dnsserver.Handler` sinkhole (zero + nxdomain), local PTR (RFC 6303 private-range zones, case-insensitive name matching, NXDOMAIN with EDNS0, disabled-forwards-upstream, public-PTR-forwarded), cache-hit, cache-miss-forward, the per-query outcome (ok forwarded, unresolved when all upstreams fail, upstream error relaying a SERVFAIL/REFUSED, with the matching failure counter), whitelist override, empty-question, EDNS0 pass-through, write-error branches, `query_privacy` masking (`raw`/`drop`/`subnet` applied at the single choke point so both the stats counter and the log see the masked client); `dnsserver.Server` full Start→query→Shutdown lifecycle on a real UDP port; the upstream health tracker (cooldown, failover, second-sweep retry), the DoH upstream transport (RFC 8484 POST against an httptest TLS server: happy path, non-200 and unparsable-body failover to a plain resolver, canceled-context error), the DoT listener (a real TLS query through the shared handler with the client address recorded, an untrusted client failing the handshake, a reload serving the new certificate to new connections, a failed reload keeping the current one, concurrent reloads during handshakes, the certificate status and its state precedence (expired over reload_failed over expiring), the connection limiter, shutdown before start releasing the port, and the N-way `Start` drain); the `api` HTTP handlers including reload single-flight, the 64 KiB body cap, `Serve`/`ListenAndServe`/`Shutdown` lifecycle, the synchronous bind-failure path, the concurrent `Serve`/`Shutdown` race guard, and the stop-before-store shutdown guard, `/healthz`, `/metrics` (including the failure counters and the wired/unwired per-upstream `shole_upstream_transport_failures_total`), `blocklist_size`, per-source health, and the `query_privacy` echo in `/api/stats`, the `client_names` `clientLabeler` (exact-over-CIDR, most-specific-CIDR, masked-network-address, empty and unparseable values) and the resolved `label` attached to `/api/stats` top clients and `/api/queries` rows, the `/api/queries` search filters (`?domain=` substring, `?client=` exact, `?blocked=`, `?outcome=`) and `parseQueryFilter`, the `/api/queries/export` streaming handler (CSV shape and headers, CSV formula-injection escaping, JSON envelope and filter echo, filter reuse, bad-format rejection, the db-disabled empty file, the privacy-mode filename/header, and the 429 concurrency guard), the `dot` object in `/api/stats` (off, on, and expired with negative days) and the DoT certificate metrics (present only when DoT is on), `/api/check` (blocked/whitelisted/allowed decisions, bad-domain rejection, and the no-stats-side-effect guarantee), `/api/top-blocked` (real-DB ordering and the db-disabled empty-list path), malformed-input rejection, encoder-error branch.
...
- **Server lifecycle test** binds the production `dnsserver.Server` to a free port (UDP + TCP), confirms a real `dns.Client.Exchange` round-trips through the handler, and verifies `Shutdown` causes `Start` to return. The DoT tests (`dot_test.go`) also bind real listeners: a full UDP, TCP, and DoT server on free ports, queried over TLS.
...
|---|----------|-------|--------|
| 1 | Should we support DNS-over-HTTPS upstream forwarding? Some ISPs intercept plain DNS on port 53. | n/a | **Resolved**: yes, shipped (CL 85; `docs/ROADMAP.md` #5). IP-literal DoH endpoints; hostname support is a follow-up. |
