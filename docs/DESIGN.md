...
```
A reply served from the response cache adds a trailing ` CACHED` token (ALLOW lines only; a blocked query never reaches the cache). Suitable for `grep`, `tail -f`, and log rotation via external tools (e.g. `logrotate`).
...
| `/api/queries` | GET | Recent queries from SQLite (`?limit=N`, default 50, capped at 1000; filters `?domain=` substring, `?client=` exact, `?blocked=true`/`false`) |
...
| `/api/history` | GET | Per-bucket total, blocked, and cached (cache-hit) query counts over a window from SQLite (`?window=24h&bucket=1h`, defaults 24h/1h, bucket count capped at 1000); empty series when `query_db` is unset |
