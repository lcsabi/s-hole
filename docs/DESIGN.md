...
- **`/debug/pprof/*`** when enabled (`enable_pprof: true`) reveals goroutine stacks, heap layouts, and binary symbols, which are useful for incident response and dangerous if exposed to the LAN. The startup log fires a WARN when it is on; operators should pair it with `api_listen: "127.0.0.1:8080"`.
