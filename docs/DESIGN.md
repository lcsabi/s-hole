...
`Snapshot(topN int)` returns a `Summary` struct with json tags, making it directly serialisable by the REST API without coupling the stats package to any HTTP library. Fields include uptime, totals, block percentage, local-PTR count (`local_ptr_count` — queries answered locally per RFC 6303), cache hit count and percentage, and top-N entry lists.
