...
| 28 | Validate the upstreams at config time (format check + single-upstream note) | Low | done (CL 81) |
...
## 28. Validate the upstreams at config time (format check + single-upstream note) (done, CL 81)

**Shipped in CL 81:** both parts, split across the two config seams. `config.Load`
