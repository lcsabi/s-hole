...
- **Unit tests:** Every implementation package under `internal/` ships a `*_test.go` file. Line coverage by package: `stats`, `config`, and `version` 100 %; `cache` 94.4 %; `api` 90.6 %; `blocklist` 92.9 %; `dnsserver` 90.3 %; `querylog` 85.7 %. The `cmd/s-hole` package is at 41.7 %; the rest is the `main()` bootstrap and signal-dispatch goroutine, which require running the binary. Module-wide coverage is 81.5 % (measured with `go test -coverpkg=./... ./...` on Linux).
