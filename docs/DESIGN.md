...
| `/api/history` | GET | Per-bucket total and blocked query counts over a window from SQLite (`?window=24h&bucket=1h`, defaults 24h/1h, bucket count capped at 1000); empty series when `query_db` is unset |
