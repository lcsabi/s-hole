...
- **Unit tests:** Every implementation package under `internal/` ships a `*_test.go` file. Line coverage by package: `stats`, `config`, and `version` 100 %; `cache` 94.8 %; `api` 89.7 %; `blocklist` 90.4 %; `dnsserver` 88.5 %; `querylog` 85.7 %. The `cmd/s-hole` package is at 31.6 %; the rest is the `main()` bootstrap and signal-dispatch goroutine, which require running the binary. Module-wide coverage is 77.9 % (measured with `go test -coverpkg=./... ./...` on Linux).
