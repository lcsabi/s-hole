...
- **Benchmark:** `BenchmarkStore_IsBlocked` and `BenchmarkStore_IsBlocked_Miss` against a 100 000-entry store guard the hot DNS path against accidental O(n) regressions — the `_Miss` variant walks a deep, not-blocked name to the TLD (the suffix walk's worst case and the common allowed-query path).
